...
use Zhanhd\ReqRes\Account\Signup\Response as SignupResponse,
    ...
    Zhanhd\ReqRes\Completion\Poll\Request,
...
$robot->on(9, function($c, $qargv) use ($config) {
    $req = new Request;
    $req->type->intval($config->type);
    $c->send(Robot::encodeWithHeader(265, $req));
});

/* on completion poll response */
$robot->on(266, function($c, $qargv){
    $res = new Response;
    $res->decode($qargv);
    print_r($res);
});
