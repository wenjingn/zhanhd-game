...
use Zhanhd\ReqRes\Account\Signup\Response as SignupResponse,
    ...
    Zhanhd\ReqRes\DepositReward\Request,
...
$robot->on(9, function($c, $qargv) use ($config) {
    $req = new Request;
    $req->drid->intval($config->drid);
    $c->send(Robot::encodeWithHeader(129, $req));
});

/**
 * on deposit reward response
 */
$robot->on(130, function($c, $qargv) {
    $res = new Response;
    $res->decode($qargv);
    print_r($res);
});
