...
use Zhanhd\ReqRes\Account\Signup\Response as SignupResponse,
    ...
    Zhanhd\ReqRes\Building\Collect\Request,
    Zhanhd\ReqRes\Building\Collect\Response;
...
$robot->on(9, function($c, $qargv) use ($config) {
    $req = new Request;
    $req->bid->intval($config->bid);
    $c->send(Robot::encodeWithHeader(7, $req));
});

/**
 * on collect response
 */
$robot->on(8, function($c, $qargv) {
    $res = new Response;
    $res->decode($qargv);
    print_r($res);
});
