...
use Zhanhd\ReqRes\Account\Signup\Response as SignupResponse,
    ...
    Zhanhd\ReqRes\Guild\Create\Request,
...
$robot->on(9, function($c, $qargv) use ($config) {
    $req = new Request;
    $req->name->strval($config->name);
    $c->send(Robot::encodeWithHeader(188, $req));
});

/* on guild create response */
$robot->on(189, function($c, $qargv) {
    $res = new Response;
    $res->decode($qargv);
    print_r($res);
});
