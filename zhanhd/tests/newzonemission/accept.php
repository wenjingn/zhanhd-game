...
use Zhanhd\ReqRes\Account\Signup\Response as SignupResponse,
    ...
    Zhanhd\ReqRes\NewzoneMission\Accept\Request,
...
$robot->on(9, function($c, $qargv) use ($config) {
    ...
    $req->idx->intval($config->idx);
    $c->send(Robot::encodeWithHeader(157, $req));
});

/* on newzone mission accept response */
$robot->on(158, function($c, $qargv) {
    $res = new Response;
    $res->decode($qargv);
    print_r($res);
});
