...
use Zhanhd\ReqRes\Account\Signup\Response as SignupResponse,
    ...
    Zhanhd\ReqRes\WeekMission\Accept\Request,
...
$robot->on(9, function($c, $qargv) use ($config) {
    $req = new Request;
    $req->mid->intval($config->mid);
    $c->send(Robot::encodeWithHeader(162, $req));
});

/* on week mission accept response */
$robot->on(163, function($c, $qargv) {
    $res = new Response;
    $res->decode($qargv);
    print_r($res);
});
