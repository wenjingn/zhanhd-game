...
use Zhanhd\ReqRes\Account\Signup\Response as SignupResponse,
    ...
    Zhanhd\ReqRes\Relation\Communicate\Request;
...
$robot->on(9, function($c, $qargv) use ($config) {
    $strangers = explode(',', $config->strangers);    
    $req = new Request;
    $req->strangers->resize(count($strangers));
    foreach ($req->strangers as $i => $o) {
        $o->intval($strangers[$i]);
    }

    $c->send(Robot::encodeWithHeader(120, $req));
});
