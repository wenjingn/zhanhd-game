...
use Zhanhd\ReqRes\Account\Signup\Response as SignupResponse,
    ...
    Zhanhd\ReqRes\Relation\LoveReward\Request,
...
$robot->on(9, function($c, $qargv) use ($config) {
    ...
    $req->gear->intval($config->gear);
    $c->send(Robot::encodeWithHeader(70, $req));
});

/**
 * on love-reward response
 */
$robot->on(71, function($c, $qargv) {
    $res = new Response;
    $res->decode($qargv);
    print_r($res);
});
