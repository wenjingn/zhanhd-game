...
use Zhanhd\ReqRes\Account\Signup\Response as SignupResponse,
    ...
    Zhanhd\ReqRes\Relation\Blacklist\Request,
...
$robot->on(9, function($c, $qargv) use ($config) {
    $req = new Request;
    $req->pid->intval($config->pid);
    $c->send(Robot::encodeWithHeader(166, $req));
});

/* on blacklist response */
$robot->on(167, function($c, $qargv){
    $res = new Response;
    $res->decode($qargv);
    print_r($res);
});
