...
use Zhanhd\ReqRes\Account\Signup\Response as SignupResponse,
    ...
    Zhanhd\ReqRes\MessageMail\Page\Request,
...
$robot->on(9, function($c, $qargv) use ($config) {
    $req = new Request;
    $req->page->intval($config->page);
    $req->num->intval($config->num);
    $c->send(Robot::encodeWithHeader(111, $req));
});

/**
 * on email page response
 */
$robot->on(112, function($c, $qargv) {
    $res = new Response;
    $res->decode($qargv);
    print_r($res);
});
